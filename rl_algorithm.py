...
class RLAlgorithm:
    ...
    def train_agent(self, agent, state, action, reward, next_state):
        ...
        # Debug information
        print(f"\nAgent type: {agent.agent_type}, ID: {id(agent)}")
        print(f"Steps: {agent.steps}, Current LR: {agent.current_lr}")
        print(f"Memory size: {len(agent.memory)}, Batch size: {current_batch_size}")
        ...
        try:
            ...
            print(f"Loss value: {agent.current_loss}")
            ...
            print(f"Updated LR: {agent.current_lr}")
            print(f"Optimizer LR: {agent.optimizer.param_groups[0]['lr']}")
